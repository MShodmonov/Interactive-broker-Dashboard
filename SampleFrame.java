...
import TestJavaClient.utils.BarList;
...
public class SampleFrame extends JFrame implements EWrapper {
    ...
    private AtomicInteger counterMin5 = new AtomicInteger(1000000);

    private AtomicInteger counterHourly = new AtomicInteger(2000000);

    private AtomicInteger counterDaily = new AtomicInteger(3000000);
...
//    private List<>
    ...
    SampleFrame() {
        scrollingWindowDisplayPanel= new JPanel(new GridLayout(0, 1));
...
//        m_orderDlg.
    ...
    private void onHistoricalData2(ContractDetails contractDetails) {
        ...
        // req historical data
        m_client.reqHistoricalData(counterMin.incrementAndGet(), contractDetails.contract(),
                dateString, "7200 S",
                "1 min", "TRADES",
                m_orderDlg.useRTH(), m_orderDlg.formatDate(), m_orderDlg.keepUpToDate(), m_chartOptions);

        m_client.reqHistoricalData(counterMin5.incrementAndGet(), contractDetails.contract(),
                dateString, "36000 S",
                "5 mins", "TRADES",
                m_orderDlg.useRTH(), m_orderDlg.formatDate(), m_orderDlg.keepUpToDate(), m_chartOptions);

        m_client.reqHistoricalData(counterHourly.incrementAndGet(), contractDetails.contract(),
                dateString, "3 D",
                "1 hour", "TRADES",
                m_orderDlg.useRTH(), m_orderDlg.formatDate(), m_orderDlg.keepUpToDate(), m_chartOptions);

        m_client.reqHistoricalData(counterDaily.incrementAndGet(), contractDetails.contract(),
                dateString, "2 M",
                "1 day", "TRADES",
                m_orderDlg.useRTH(), m_orderDlg.formatDate(), m_orderDlg.keepUpToDate(), m_chartOptions);
...
//        m_tickers.add("History List Complete");
    }
    ...
    public void historicalDataEnd(int reqId, String startDate, String endDate) {
        ...
        if (reqId == counterMin.get()) {
            drawCandleSticks(minList.getBarList(), minChart);
//            scrollingWindowDisplayPanel.add(minChart);
        } else if (reqId == counterMin5.get()) {
            drawCandleSticks(min5List.getBarList(), min5Chart);
//            scrollingWindowDisplayPanel.add(min5Chart);
        } else if (reqId == counterHourly.get()) {
            drawCandleSticks(hourlyList.getBarList(), hourlyChart);
//            scrollingWindowDisplayPanel.add(hourlyChart);
        } else if (reqId == counterDaily.get()) {
            drawCandleSticks(dailyList.getBarList(), dailyChart);
//            scrollingWindowDisplayPanel.add(dailyChart);
        }
    }

    public void drawCandleSticks(List<Bar> barList, SampleCandlestick chart){
        AtomicReference<Double> max = new AtomicReference<>(0D);
        AtomicReference<Double> min = new AtomicReference<>(barList.get(0).low());
        DefaultHighLowDataset dataset = new DefaultHighLowDataset("ES FUT", barList.stream().map(bar -> {
...
//        chart.pack();
//        RefineryUtilities.centerFrameOnScreen(minChart);
